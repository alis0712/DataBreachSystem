...
/**
 *
 * @author syeda code reference https://github.com/RameshMF/jsp-servlet-jdbc-mysql-crud-tutorial/blob/master/src/net/javaguides/usermanagement/dao/UserDAO.java
 */
public class UserDAO {

    //add user details in userinfo : RegistrationForm.java
    public boolean AddUser(String loginID, String password, String firstName, String lastName, long cellNo, String emailId, String rollId, String city, String zipcode) {
//            String ipAddrStr = LoginForm.userInfo.getIpAddrStr();
        String ipAddrStr = "";
    
            ipAddrStr = NetworkHelper.getMACAddress();
       
        String query = "insert into userinfo (`LoginId`, `UserPassword`,  `FName`, `LName`, `IPAddress`, `CellNumber`, `EmailAddress`, `ActiveFlag`, `LogStatus`, `LogOnTime`, `LogOffTime`,`rollId`,`city`,`zipcode` )" 
                + " values ('" + loginID + "','" + password + "','" + firstName + "','" + lastName + "','" + ipAddrStr + "','" + cellNo + "','" 
                + emailId + "',1,'Online'," + GetSetProperties.getProperty("CURR_TIMESTAMP") + ",null,'" + rollId + "','"+ city + "','" + zipcode + "')";
...
//            String ipAddrStr = LoginForm.userInfo.getIpAddrStr();
        ...
        {
           JOptionPane.showMessageDialog(null, "Error", "Please Login", JOptionPane.ERROR_MESSAGE);
            return false;
        }
...
//            new UserDAOHelper().executeInsert(query);
    ...
    public List getUsers()
    {
        ...
        else
        {
            ...
            list = ConnectionManager.getBeanList(UserModel.class, query);
        }
    ...
    public List getUsers(String UserID)
    {
        ...
        {
            String query = "select * from userinfo where ActiveFlag=1 and rollId=" + UserID ;
            list = ConnectionManager.getBeanList(UserModel.class, query);
        }
        return list;
    }
    ...
    public List populateFiles2(JTable jtable) throws Exception
    {
        String query = "SELECT b.FolderPath, b.TotalSize, b.Receiver,date_format( b.SaveTimestamp, '%d-%b-%y %a') as SaveTimestamp " +
                ",u.FName , u.LName,originalPath ,u.`EmailAddress`, u.`CellNumber`,"
                + "b.`description`,b.`ID` FROM datadb.backupinfo b,datadb.userinfo u where u.userid=b.sender and b.Receiver =" + UserSession.loggedInUser.getUserId();
        System.out.println("query " + query);
        List list = ConnectionManager.getMapList(query);
